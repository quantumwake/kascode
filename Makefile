...
test: ## run parser + protocol + characterization tests (no model needed)
	...
	@uv run python tests/test_ports.py
	@uv run python tests/test_architecture.py
	@uv run python tests/test_backends.py
	...
	@uv run python tests/test_ai_wellbeing.py
	@uv run python tests/test_spec.py
...
uninstall: ## remove the kas tool (KAS_PURGE=1 also drops ~/.kascode)
	@./uninstall.sh

doctor: ## detect platform/GPU + report what each capability needs (ARGS="--install")
	@uv run python scripts/doctor.py $(ARGS)

smoke-serve: ## smoke-test the RUNNING server: streaming + tool calling (green/red)
