"""The MLX backend: an EngineLike adapter for Apple-silicon GPUs via mlx_lm.

This is ONE backend behind the EngineLike port (see server/core/ports.py and the
server/backends/ factory). A llama.cpp/GGUF, CUDA, or ROCm backend is a sibling
module implementing the same protocol — nothing MLX-specific leaks past this file
or the dialect it picks. All MLX work runs on one dedicated thread:
...
Why this backend is one cohesive module (not split like the agent's tui/ package):
every operation here — load, slot/KV-cache management, generation, on-disk KV
...
same thread-local state, so one file aids reasoning; splitting would scatter
intimately-coupled code for no gain. Public surface = the EngineLike protocol
(tokenize/encode/generate/swap/cache_snapshot/rehydrate + stats); `_`-prefixed is
worker-internal. Backend-neutral types (GenChunk) live in core.ports, not here.
"""
...
class MlxEngine:
    ...
    def _worker(self) -> None:
        try:
            ...
            # generation stream is registered here. Gate it: MLX is Apple-Silicon
            # only (Metal/Accelerate), so on the wrong host — or if the optional
            # mlx-lm isn't installed — surface a clear, actionable error instead
            # of a bare ImportError from deep in the stack. (make_engine already
            # platform-checks before constructing us; this guards direct use.)
            try:
                ...
                raise RuntimeError(
                    f"the MLX backend requires Apple Silicon (macOS/arm64) with mlx-lm "
                    f"installed, but importing it failed: {exc}. On other hardware run "
                    "the agent against a remote --base-url, or select a non-MLX backend "
                    "(KAS_BACKEND)."
                ) from exc
    ...
    def _load_model(self, model_id: str) -> None:
        ...
        try:
            ...
            self.n_layers = None
        self.dialect = detect_dialect(getattr(self.tokenizer, "chat_template", None), self.model_id)
        self._slots = {}
        self._slot_order = []
